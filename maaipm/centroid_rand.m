...
% generate random centroid using mvnrnd() [default] or kmeans()
  ...
  elseif strcmp(init_method, 'kmeans')
    %%[idx, c.supp] = kmeans(supp', support_size,'MaxIter',1000,'Display','final','Options',statset('UseParallel',1));
    [idx, c.supp] = kmeans(supp', support_size,'MaxIter',1000,'Display','final');
    %用kmeans初始化m个support points， k-means++ algorithm for cluster center initialization
    %By default, kmeans uses the squared Euclidean distance metric and the k-means++ algorithm for cluster center initialization.
    c.supp = c.supp';
    c.w = histc(idx, 1:support_size);%初始化w
    c.w = c.w' / sum(c.w);
  end
